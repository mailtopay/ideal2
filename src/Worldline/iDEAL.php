...
readonly class iDEAL
{
    ...
    public function retrieveAccessToken(): AccessToken
    {
        ...
        $bankName   = preg_replace('/[^a-zA-Z0-9_]/', '', get_class($this->config->getBank()));
        $cacheKey   = $this->config->getCachePrefix() . '.accessToken.'.$bankName.'.'.$this->config->getMerchantId();
    ...
    public function verifyCallbackResponse(string $callbackResponse, array $headers = null): bool
    {
        ...
        $headersGiven = sprintf("messagecreatedatetime: %s\nx-request-id: %s\ndigest: %s", $dateTime, $requestId, $digest);
